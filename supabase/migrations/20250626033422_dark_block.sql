...
-- Create admin policy for stripe_customers using direct admin_users table check
DO $$ BEGIN
...
EXCEPTION
  WHEN others THEN
    -- If there's any error, just skip the admin policy
    NULL;
END $$;
...
-- Create admin policy for stripe_subscriptions using direct admin_users table check
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'admin_users' AND table_schema = 'public') THEN
...
EXCEPTION
  WHEN others THEN
    -- If there's any error with default data creation, just continue
    NULL;
END $$;